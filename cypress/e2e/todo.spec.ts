describe('Todo page interactions', () => {
  ...
  it('shows empty state and adds a new todo', function () {
    cy.visit('/todos');

    cy.wait('@getAccount');
    cy.wait('@listDocumentsEmpty');

    cy.contains('No tasks yet. Start by adding one above!').should('be.visible');

    // stub createDocument to succeed
    cy.intercept('POST', '**/v1/databases/**/collections/**/documents', (req) => {
      req.reply({
        statusCode: 201,
        body: {
          $id: 'todo_1',
    ...
    cy.get('input[placeholder="What needs to be done?"]').type('New Cypress Task');
